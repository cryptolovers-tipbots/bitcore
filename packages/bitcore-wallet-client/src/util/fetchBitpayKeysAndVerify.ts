...
keyRequests.push(
  (() => {
    ...
    return request({
      ...
      headers: {
        'user-agent': 'Astracore Key-Check Utility',
      },
      json: true,
    }).then((pgpKeyFiles) => {
...
Promise.all(keyRequests)
  .then(() => {
    ...
    if (bitpayOnlyKeys.length) {
      console.log('Astracore returned some keys which are not present in github');
    ...
    if (githubOnlyKeys.length) {
      console.log('GitHub returned some keys which are not present in Astracore');
  ...
  .then(() => {
    ...
    return request({
      ...
      if (rawEccPayload.indexOf('rate limit') !== -1) {
        return Promise.reject('Rate limited by Astracore');
      }
    ...
    if (signatureCount >= Object.keys(bitpayPgpKeys).length / 2) {
      ...
      parsedEccPayload.publicKeys.forEach((pubkey) => {
        ...
        keyMap['mh65MN7drqmwpCRZcEeBEE9ceQCQ95HtZc'] = {
          owner: 'Astracore (TESTNET ONLY - DO NOT TRUST FOR ACTUAL BITCOIN)',
          networks: ['test'],
