...
var Astracore_ = {
  btc: CWC.AstracoreLib,
  bch: CWC.AstracoreLibCash,
  eth: CWC.AstracoreLib,
  xrp: CWC.AstracoreLib,
};
...
export class API extends EventEmitter {
  ...
  // Expose astracore
  static Astracore = CWC.AstracoreLib;
  static AstracoreCash = CWC.AstracoreLibCash;
  ...
  validateKeyDerivation(opts, cb) {
    ...
    var testHardcodedKeys = () => {
      ...
      var xpub = Astracore.HDPublicKey.fromString(
    ...
    var testLiveKeys = () => {
      ...
      if (!xpriv) {
        xpriv = new Astracore.HDPrivateKey(c.xPrivKey);
      }
      xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
      var xpub = new Astracore.HDPublicKey(c.xPubKey);
  ...
  decryptBIP38PrivateKey(encryptedPrivateKeyBase58, passphrase, opts, cb) {
    ...
    var privateKey = new Astracore.PrivateKey(privateKeyWif);
    var address = privateKey.publicKey.toAddress().toString();
    var addrBuff = Buffer.from(address, 'ascii');
    var actualChecksum = Astracore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
    var expectedChecksum = Astracore.encoding.Base58Check.decode(encryptedPrivateKeyBase58)
      .toString('hex')
      .substring(6, 14);
  ...
  static _buildSecret(walletId, walletPrivKey, coin, network) {
    if (_.isString(walletPrivKey)) {
      walletPrivKey = Astracore.PrivateKey.fromString(walletPrivKey);
    }
    var widHex = Buffer.from(walletId.replace(/-/g, ''), 'hex');
    var widBase58 = new Astracore.encoding.Base58(widHex).toString();
  ...
  static parseSecret(secret) {
    ...
    try {
      ...
      var widHex = Astracore.encoding.Base58.decode(widBase58).toString('hex');
      var walletId = split(widHex, [8, 12, 16, 20]).join('-');

      var walletPrivKey = Astracore.PrivateKey.fromString(secretSplit[1]);
  ...
  _addSignaturesToAstracoreTxBitcoin(txp, t, signatures, xpub) {
    $.checkState(txp.coin);
    $.checkState(txp.signingMethod);
    const astracore = Astracore_[txp.coin];
    if (signatures.length != txp.inputs.length) throw new Error('Number of signatures does not match number of inputs');

    let i = 0;
    const x = new astracore.HDPublicKey(xpub);

    _.each(signatures, (signatureHex) => {
      try {
        const signature = astracore.crypto.Signature.fromString(signatureHex);
        ...
        const s = {
          ...
          sigtype:
            ...
            astracore.crypto.Signature.SIGHASH_ALL | astracore.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub,
        };
  ...
  _addSignaturesToAstracoreTx(txp, t, signatures, xpub) {
    ...
    switch (chain) {
      ...
      case 'ETH':
        ...
        break;
      default:
        return this._addSignaturesToAstracoreTxBitcoin(txp, t, signatures, xpub);
    }
  }
  ...
  _applyAllSignatures(txp, t) {
    ...
    _.each(sigs, (x) => {
      this._addSignaturesToAstracoreTx(txp, t, x.signatures, x.xpub);
    });
  }
  ...
  createWallet(walletName, copayerName, m, n, opts, cb) {
    ...
    var walletPrivKey = opts.walletPrivKey || new Astracore.PrivateKey();
    ...
    var args = {
      ...
      m,
      n,
      pubKey: new Astracore.PrivateKey(walletPrivKey).toPublicKey().toString(),
      coin,
      network,
  ...
  recreateWallet(cb) {
    ...
    this.getStatus(
      ...
      (err) => {
        ...
        var c = this.credentials;
        var walletPrivKey = Astracore.PrivateKey.fromString(c.walletPrivKey);
  ...
  addAccess(opts, cb) {
    ...
    opts = opts || {};
    var requestPubKey = new Astracore.PrivateKey(opts.requestPrivKey).toPublicKey().toString();
  ...
  static upgradeMultipleCredentialsV1(oldCredentials) {
    ...
    if (newKeys.length > 0) {
      ...
      let credGroups = _.groupBy(newCrededentials, (x) => {
        ...
        let xpub = new Astracore.HDPublicKey(x.xPubKey);
