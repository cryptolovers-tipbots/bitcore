...
helpers._generateCopayersTestData = function () {
  ...
  _.each(xPrivKeys, function (xPrivKeyStr, c) {
    var xpriv = Astracore.HDPrivateKey(xPrivKeyStr);
    var xpub = Astracore.HDPublicKey(xpriv);

    var xpriv_45H = xpriv.deriveChild(45, true);
    var xpub_45H = Astracore.HDPublicKey(xpriv_45H);
    ...
    var xpub_44H_0H_0H = Astracore.HDPublicKey(xpriv_44H_0H_0H);
    ...
    var xpub_1H = Astracore.HDPublicKey(xpriv_1H);
...
helpers.randomTXID = function () {
  return Astracore.crypto.Hash.sha256(Buffer.from((Math.random() * 100000).toString())).toString('hex');
};
...
helpers.stubUtxos = function (server, wallet, amounts, opts, cb) {
  ...
  var S = Astracore_[wallet.coin].Script;
  async.waterfall(
    [
      function (next) {
...
helpers.clientSign = function (txp, derivedXPrivKey) {
  ...
  var xpriv = new Astracore.HDPrivateKey(derivedXPrivKey, txp.network);
  ...
  switch (txp.coin) {
    ...
    case 'xrp':
      ...
      let tx = ChainService.getAstracoreTx(txp).uncheckedSerialize();
