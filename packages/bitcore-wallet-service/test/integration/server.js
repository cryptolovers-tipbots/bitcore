...
describe('Wallet service', function () {
    ...
    it('should check m-n combination', function (done) {
      ...
      async.eachSeries(
        ...
        function (pair, cb) {
          ...
          var pub = new Astracore.PrivateKey().toPublicKey();
          opts.m = pair.m;
          opts.n = pair.n;
  ...
  describe('Multiple request Pub Keys', function () {
    ...
    beforeEach(function () {
      reqPrivKey = new Astracore.PrivateKey();
    ...
    describe('#addAccess 1-1', function () {
      ...
      it('should fail to access with wrong privkey after gaining access', function (done) {
        ws.addAccess(opts, function (err, res) {
          ...
          server.getBalance(res.wallet.walletId, function (err, bal) {
            should.not.exist(err);
            var privKey = new Astracore.PrivateKey();
  ...
  _.each(testSet, function (x) {
    ...
    describe('#createTx ' + coin + ' flags' + JSON.stringify(flags), function () {
      describe('Tx proposal creation & publishing ' + coin, function () {
        ...
        describe('Validations', function () {
          ...
          if (!flags.noUtxoTests) {
            ...
            it('should be able to specify change address', function (done) {
              helpers.stubUtxos(server, wallet, [1, 2], function (utxos) {
                ...
                server.createTx(txOpts, function (err, tx) {
                  should.not.exist(err);
                  should.exist(tx);
                  var t = ChainService.getAstracoreTx(tx);
            ...
            it('should be fail if specified change address is not from the wallet', function (done) {
              helpers.stubUtxos(server, wallet, [1, 2], function (utxos) {
                var addr = new Astracore_[coin].PrivateKey().toAddress();
                var txOpts = {
                  outputs: [
                    {
                      toAddress: addressStr,
        ...
        it('should support creating a tx with no change address', function (done) {
          helpers.stubUtxos(server, wallet, [1, 2], { coin }, function () {
            ...
            server.createTx(txOpts, function (err, txp) {
              should.not.exist(err);
              should.exist(txp);
              var t = ChainService.getAstracoreTx(txp).toObject();
        ...
        it('should fail gracefully when astracore throws exception on raw tx creation', function (done) {
          helpers.stubUtxos(server, wallet, 1, { coin }, function () {
            ...
            var astracoreStub;
            var astracoreStub = sandbox.stub(CWC.AstracoreLib, 'Transaction');
            astracoreStub.throws({
              name: 'dummy',
              message: 'dummy exception',
            });
            var astracoreStub = sandbox.stub(CWC.AstracoreLibCash, 'Transaction');
            astracoreStub.throws({
              name: 'dummy',
              message: 'dummy exception',
            });
            ...
            server.createTx(txOpts, function (err, tx) {
              ...
              if (astracoreStub) astracoreStub.restore();
              cwcStub.restore();
              done();
            });
          });
          ...
          it('should create tx with 0 change output', function (done) {
            helpers.stubUtxos(server, wallet, 1, function () {
              ...
              server.createTx(txOpts, function (err, tx) {
                should.not.exist(err);
                should.exist(tx);
                var astracoreTx = ChainService.getAstracoreTx(tx);
                astracoreTx.outputs.length.should.equal(1);
                astracoreTx.outputs[0].satoshis.should.equal(tx.amount);
                done();
              });
            });
          });
        ...
        if (!flags.noUtxoTests) {
          ...
          it('should accept a tx proposal signed with a custom key', function (done) {
            var reqPrivKey = new Astracore.PrivateKey();
            ...
            helpers.stubUtxos(server, wallet, 1, function () {
              ...
              server.createTx(txOpts, function (err, txp) {
                ...
                server.createTx(txOpts, function (err, txp) {
                  ...
                  t = ChainService.getAstracoreTx(txp);
        ...
        it('should correct fee if resulting change would be below threshold', function (done) {
          helpers.stubUtxos(server, wallet, ['200bit', '500sat'], function () {
            ...
            server.createTx(txOpts, function (err, txp) {
              ...
              var changeOutput = ChainService.getAstracoreTx(txp).getChangeOutput();
        ...
        function (err) {
          ...
          server.getTx(
            ...
            function (err, txp) {
              ...
              should.exist(txp.raw);
              const tx = new Astracore.Transaction(txp.raw);
              clock.restore();
              done();
            }
          );
  ...
  describe('Simplex', () => {
    ...
    describe('#simplexGetQuote', () => {
      ...
      it('should return error if post returns error', () => {
        const fakeRequest2 = {
          post: (_url, _opts, _cb) => {
            return _cb(new Error('Error'), null);
          },
        };
