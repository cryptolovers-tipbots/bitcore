          ...
          start(TestExpressApp, function (err, data) {
            ...
            request(requestOptions, function (err, res, body) {
              if (config.maintenanceOpts.maintenanceMode === true) {
                ...
                body.should.equal(
                  `{"code":503,"message":"Astracore Wallet Service is currently under maintenance. Please periodically check https://status.bitpay.com/ to stay up to date with our current status."}`
                );
              } else {
                should.not.exist(err);
                res.statusCode.should.equal(200);
