...
var Point = astracore.crypto.Point;
...
describe('PrivateKey', function () {
  ...
  describe('#toAddress', function () {
    ...
    it('creates network specific witness address', function () {
      var pk = PrivateKey.fromWIF('cR4qogdN9UxLZJXCNFNwDRRZNeLRWuds9TTSuLNweFVjiaE4gPaq');
      pk.toAddress(Networks.livenet, Address.PayToWitnessPublicKeyHash).network.name.should.equal(
        Networks.livenet.name
      );
      pk.toAddress(Networks.testnet, Address.PayToWitnessPublicKeyHash).network.name.should.equal(
